...
const VendorAddProduct = () => { 
  ...
  const [vendorData, setVendor] = useState<{ id: string; status: string } | null>(null);
  ...
  const addColor = () => {
    ...
    if (colors.map(c => c.toUpperCase()).includes(normalizedColor)) {
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    try {
      ...
      // Insert product matching your Product interface
        ...
        <div className="bg-white rounded-xl shadow-lg p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-6">
            {vendorStatus === 'pending' 
              ? 'Your vendor account is still under review. You can add products once approved.'
